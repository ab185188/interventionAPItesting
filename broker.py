...
class Broker:
    ...
    def publish(self, topic, payload=None, properties=None, content_type = 'application/json' ):

        properties=mqtt.Properties(PacketTypes.PUBLISH)

        properties.ContentType=content_type

        return self.mqtt_client.publish(topic=topic, payload=payload, qos=1, retain=False, properties=properties)
    ...
    def publishFile(self, topic, filename,  content_type = 'application/json') :
        print('------------------------------------------------------------')
        ...
        #
        mqttInfo = self.publish(topic, payload=payload, content_type=content_type)
    ...
    def get_message(self, num_seconds=5 ):
        ...
        while num_seconds != 0 or not self._new_msg_received: #wait in loop
